...
module.exports = function fsProvider (config) {
  ...
  let provider = {
    ...
    get: function (key, cb) {
      ...
      fs.readFile(join(directory, key), { encoding: 'utf8' }, function (err, value) {
    ...
    set: function (key, value, cb) {
      ...
      fse.outputFile(join(directory, key), strValue, { encoding: 'utf8' }, (err) => {
    ...
    list: function (before, cb) {
      ...
      fs.readdir(directory, { encoding: 'utf8' }, (err, items) => {
        ...
        let entries = items
          ...
          .filter((key) => {
            return new RegExp('^' + before).test(key)
          })
        cb(null, entries)
      })
    }
